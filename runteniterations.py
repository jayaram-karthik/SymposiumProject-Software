...
def run_additional_iteration(pred, inflow_1, inflow_2, outflow_1, outflow_2, volfrac):
    ...
    controls = File(f"fiveiterationtest/output/input-{inflow_1}-{inflow_2}/output-{outflow_1}-{outflow_2}/control_iterations_guess.pvd")
    allctrls = File(f"fiveiterationtest/output/input-{inflow_1}-{inflow_2}/output-{outflow_1}-{outflow_2}/allcontrols.pvd")
    ...
    controls = File(f"fiveiterationtest/controloutput/input-{inflow_1}-{inflow_2}/output-{outflow_1}-{outflow_2}/fraction{volfrac}/control_iterations_final.pvd")
    rho_viz = Function(A, name="ControlVisualisation")
    def eval_cb(j, rho):
        rho_viz.assign(rho)
        controls << rho_viz
        allctrls << rho_viz
        with open(f'./dataforpaper/fiveiterationcallback/modeltest_in-{inflow_1}-{inflow_2}_out-{outflow_1}-{outflow_2}_{volfrac}.txt', 'w+') as f:
...
# from the solution of :math:`q=0.01`:
    ...
    rho_opt_final = XDMFFile(MPI.comm_world, f"fiveiterationtest/finaloutputs/input-{inflow_1}-{inflow_2}/output-{outflow_1}-{outflow_2}/control_solution_final_{volfrac}.xdmf")
...
for filename in topologies:
    ...
    with open(f'./dataforpaper/fiveiterationexectime/modeltest_in-{inflow[0]}-{inflow[1]}_out-{outflow[0]}-{outflow[1]}_{volfrac}.txt', 'w+') as f:
